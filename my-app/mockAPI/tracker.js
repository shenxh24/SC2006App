...
import { searchFood, getFoodNutrition } from '../services/trackerService';
import '../App.css';
...
function Tracker({ dailyGoals: initialDailyGoals = null }) {
  ...
  // Helper function to get nutrient values
  ...
  const handleAddFood = async (e) => {
    ...
    try {
      // MOCK: Replace actual API calls
      console.log("MOCK: Bypassing API calls");
      
      const mockFoodItem = {
        id: Date.now(),
        foodName: foodName, // Use the input directly
        amount: `${amount} ${unit}`,
        calories: amount * 1.65, // Mock values based on chicken breast
        protein: amount * 0.31,
        fat: amount * 0.036,
        carbs: 0,
        lessOilSalt,
      ...
      setFoodItems(prevItems => [...prevItems, mockFoodItem]);
      setFoodName('');
      setAmount('');
      setUnit('grams');
      ...
      setError("Mock mode active - no real API calls made");
